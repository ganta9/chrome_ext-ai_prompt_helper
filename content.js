...
async function init() {
  ...
  // プロンプトデータ初期確認（Service Worker依存を削除）
  try {
    chrome.storage.local.get(['promptsCache'], (result) => {
      if (result.promptsCache && result.promptsCache.length > 0) {
        console.log('ローカルプロンプトデータ確認:', result.promptsCache.length + '件');
      } else {
        console.log('ローカルプロンプトデータなし - ポップアップから手動更新してください');
      }
    });
  } catch (error) {
    console.warn('ローカルストレージアクセスエラー:', error);
  }
...
async function openPromptPanel() {
  try {
    ...
    // ローカルストレージからプロンプトデータを取得
    chrome.storage.local.get(['promptsCache'], (result) => {
      const prompts = result.promptsCache || [];
      console.log('ローカルプロンプトデータ取得:', prompts.length + '件');

      if (prompts.length === 0) {
        showNotification('プロンプトデータがありません。拡張機能のポップアップから「更新」をクリックしてください。', 'warning');
        return;
      }
...
async function getGitHubPagesUrl() {
  try {
    ...
    return result['github-pages-url'] || 'https://ganta9.github.io/chrome_ext-ai_prompt_helper/';
  } catch (error) {
